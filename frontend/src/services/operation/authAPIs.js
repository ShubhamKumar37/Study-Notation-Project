    ...
    try {
      ...
      dispatch(
        setUser({
          ...
          image: response.data.userExist.image,
        })
      );
...
export function getPasswordResetToken(email, setEmailSent) {
  return async (dispatch) => {
    dispatch(setLoading(true));
    try {
      const response = await apiConnector("PUT", RESET_PASSWORD_TOKEN_USER, {
        email,
      });
      ...
      toast.error(
        `May be this user ${email} doesnot exist, check entered email`
      );
    }
...
export function resetPassword(password, confirmPassword, token, navigate) {
  return async (dispatch) => {
    ...
    try {
      const response = await apiConnector("PUT", RESET_PASSWORD_USER, {
        password,
        confirmPassword,
        token,
      });
